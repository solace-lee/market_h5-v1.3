          ...
          <div class="select-area" v-on:click.stop="nothing">
            ...
            <div class="price-range">
              ...
              <div class="price-range-container">
                <input type="number" class="min-price" v-model="beginPrice" placeholder="最低价">
                <div class="link">——</div>
                <input type="number" class="max-price" v-model="endPrice" placeholder="最高价">
              </div>
            </div>
            <!-- 地区 -->
            <div class="choose-region">
              ...
              <div class="btn-container">
                <div class="btn-btn">
                  <div class="btn-left" @click.stop="resetBtn">重置</div>
                  <div class="btn-right" @click.stop="confirm">确定</div>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      beginPrice: null, // 筛选中的最低价格
      endPrice: null, // 筛选中的最高价格
      config: {}, // 全局筛选条件
  ...
  watch: {
    ...
    $route: function(to, from) {
      if (from.name == "详情页") {
        this.goHistory();
      } else if (from.name == "category" || from.name == "Car") {
        this.readHistory();
        this.haskey();
      } else if (from.name == "Category2") {
        // this.haskey()
        return;
      } else {
        this.cleanInput();
  ...
  methods: {
    ...
    // 选中分类
    selectCate(cateId) {
      if (this.thirdClassifyId == cateId) {
    ...
    confirm() {
      ...
      this.config.beginPrice = min;
      this.config.endPrice = max;
      this.config.page = 1;
