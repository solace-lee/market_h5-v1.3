      ...
      <div class="form">
        <el-form :model="ReginForm" ref="ReginForm" :rules="rule" label-width="0">
          ...
          >
            ...
            <input
              v-if="item.name==='请输入要绑定的手机号'"
              :type="item.type"
              v-model.number="ReginForm.userPhone"
              :placeholder="item.name"
            >
            <input
              v-if="item.name==='请设置登录密码'"
              :type="item.type"
          ...
          <el-form-item>
            <el-button type="success" class="btn" @click="submit">立即绑定手机</el-button>
            <el-button type="success" class="btn" @click="goback">使用手机号码登录</el-button>
          </el-form-item>
        </el-form>
  ...
  methods: {
        ...
        .then(res => {
          if (res.status == 200) {
            if (res.data.status == 200) {
              ...
              Toast('该号码已存在，请用手机号码登录再绑定微信')
              localStorage.setItem('Account', tel)
              this.$router.replace({
                path: '/login?stop=1'
              })
            } else {
    ...
    thisAjax() {
      ...
      if (this.localURL[0] == 'file') {
        obj.wxUnionid = this.userInfo.unionid
        obj.headimgurl = this.userInfo.headimgurl
        ...
        .then(res => {
          if (res.data.status === 200) {
            Toast("绑定手机号码成功")
              ...
              if (this.localURL[0] == 'file') {
                config = {
                  wxUnionid: this.userInfo.unionid
                }
              } else {
                config = {
