    ...
    count(){
      ...
      if(this.$store.getters.selectedList==undefined) { 
        return 0
          ...
          value += this.$store.getters.selectedList[i].value;
        }
        return value;
    ...
    allpay(){
      ...
      if(this.$store.getters.selectedList) {
        ...
        for (let i = 0; i < this.$store.getters.selectedList.length; i++) {
          ...
          all += this.$store.getters.selectedList[i].price*this.$store.getters.selectedList[i].value;
  ...
  methods:{
    ...
    goPay(){
        ...
        if(this.$store.getters.selectedList.length) {
          ...
          this.$router.push({path:'/cartpay?name='+'确认订单'})
